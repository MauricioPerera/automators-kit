    ...
    groups: [
      ...
      },
      {
        ...
        endpoints: [
          ...
          { method: 'GET', path: '/api/workflows/:id', auth: 'bearer, project viewer+ if the workflow has a projectId (open to any authenticated user if unassigned)', description: 'Get workflow by ID' },
          ...
          { method: 'POST', path: '/api/workflows/:id/toggle', auth: 'bearer, project editor+ if the workflow has a projectId (open to any authenticated user if unassigned)', description: 'Toggle workflow active/inactive' },
          { method: 'POST', path: '/api/workflows/:id/run', auth: 'bearer, project editor+ if the workflow has a projectId (open to any authenticated user if unassigned)', bodyDescription: 'Arbitrary object passed as run input, no formal schema', description: 'Run a workflow synchronously' },
          { method: 'GET', path: '/api/workflows/:id/executions', auth: 'bearer, project viewer+ if the workflow has a projectId (open to any authenticated user if unassigned)', description: 'List executions, optional ?limit (default 50) and ?status (e.g. failed)' },
          { method: 'GET', path: '/api/workflows/executions/:execId', auth: 'bearer, project viewer+ on the owning workflow if it has a projectId (open to any authenticated user if unassigned)', description: 'Get one execution' },
          { method: 'POST', path: '/api/workflows/executions/:execId/retry', auth: 'bearer', notes: 'Not yet project-gated -- same underlying gap as executions above, known follow-up, not bundled in', description: "Retry a failed execution from the DAG level where it failed, instead of re-triggering the whole workflow" },
          { method: 'GET|POST|PUT|PATCH|DELETE', path: '/api/workflows/webhook/:path', auth: 'none (X-Webhook-Secret header)', bodyDescription: 'Arbitrary JSON (GET/HEAD: query string instead), passed as the trigger payload', description: 'Webhook trigger entrypoint. Only the HTTP method the workflow\'s trigger.config.method declares (default POST) actually matches -- others 404 same as an unregistered path. Responds immediately with { triggered } by default; if trigger.config.respond is "whenFinished", responds instead with { execution } once the run stops progressing (success, failure, or a wait.* pause)' },
          ...
          { method: 'POST', path: '/api/workflows/credentials/:name/test', auth: 'admin', description: 'Verify a credential is usable (decrypts, and for OAuth2 near expiry, that a refresh actually succeeds) without running a workflow' },
