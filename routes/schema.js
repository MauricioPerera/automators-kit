    ...
    groups: [
      ...
      },
      {
        ...
        endpoints: [
          ...
          { method: 'GET', path: '/api/workflows/:id/executions', auth: 'bearer', description: 'List executions, optional ?limit (default 50) and ?status (e.g. failed)' },
          { method: 'GET', path: '/api/workflows/executions/:execId', auth: 'bearer', description: 'Get one execution' },
          { method: 'POST', path: '/api/workflows/executions/:execId/retry', auth: 'bearer', description: "Retry a failed execution from the DAG level where it failed, instead of re-triggering the whole workflow" },
