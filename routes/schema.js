    ...
    groups: [
      ...
      },
      {
        ...
        endpoints: [
          ...
          { method: 'GET|POST|PUT|PATCH|DELETE', path: '/api/workflows/webhook/:path', auth: 'none (X-Webhook-Secret header)', bodyDescription: 'Arbitrary JSON (GET/HEAD: query string instead), passed as the trigger payload', description: 'Webhook trigger entrypoint. Only the HTTP method the workflow\'s trigger.config.method declares (default POST) actually matches -- others 404 same as an unregistered path' },
