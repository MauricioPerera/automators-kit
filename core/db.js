  ...
  for (const key of Object.keys(filter)) {
    ...
    for (const op of Object.keys(cond)) {
      ...
      switch (op) {
        ...
        case '$contains': {
          if (Array.isArray(val)) { if (!val.includes(target)) return false; }
          else if (typeof val === 'string') { if (!val.includes(target)) return false; }
          else return false;
          break;
        }
        case '$containsAny': {
          if (!Array.isArray(val) || !Array.isArray(target)) return false;
          if (!target.some(t => val.includes(t))) return false;
          break;
        }
        case '$containsNone': {
          if (!Array.isArray(val) || !Array.isArray(target)) return false;
        ...
        case '$len': {
          if (typeof val !== 'string' || val.length !== target) return false;
          break;
        }
        case '$type': {
          if (target === 'array') { if (!Array.isArray(val)) return false; }
          else if (target === 'null') { if (val !== null) return false; }
          else if (target === 'date') { if (!(val instanceof Date) && isNaN(Date.parse(val))) return false; }
          else if (typeof val !== target) return false;
          break;
        }
        case '$finite': {
          if (target && !Number.isFinite(val)) return false;
          if (!target && Number.isFinite(val)) return false;
          break;
        }
        case '$elemMatch': {
...
function _getFs() {
  if (!_fs) {
    try {
      // Works in Node 20+, Bun, Deno (all support node: prefix with ESM)
      // globalThis.process.getBuiltinModule available in Node 22+, fallback to require
      if (typeof globalThis.process !== 'undefined' && globalThis.process.getBuiltinModule) {
        _fs = globalThis.process.getBuiltinModule('fs');
        _path = globalThis.process.getBuiltinModule('path');
      } else {
        // Bun and older Node: require() still works for built-in modules in ESM
        _fs   = require('fs');
        _path = require('path');
      }
    } catch {
          ...
          for (const [key, groupDocs] of groups) {
            ...
            for (const [accName, accDef] of Object.entries(stage.accumulators)) {
              if (accDef.$count) {
                ...
                const nums = groupDocs.map(d => Number(_getNestedValue(d, accDef.$min))).filter(n => Number.isFinite(n));
                result[accName] = nums.length > 0 ? Math.min(...nums) : null;
              } else if (accDef.$max) {
                const nums = groupDocs.map(d => Number(_getNestedValue(d, accDef.$max))).filter(n => Number.isFinite(n));
                result[accName] = nums.length > 0 ? Math.max(...nums) : null;
              } else if (accDef.$push) {
...
class EncryptedAdapter {
  ...
  static _getCrypto() {
    ...
    // Node.js < 19: try built-in module access
    try {
      if (typeof globalThis.process !== 'undefined' && globalThis.process.getBuiltinModule) {
        return globalThis.process.getBuiltinModule('crypto').webcrypto;
      }
...
class FieldCrypto {
  ...
  async decrypt(encoded) {
    ...
    try {
      ...
      throw new Error(`FieldCrypto: decryption failed — wrong key or corrupted data (${err.message})`);
    }
  }
...
class Auth {
  ...
  async init() {
    ...
    this._cleanupTimer = setInterval(() => this.cleanExpiredSessions(), 3600000);
  }
  ...
  async _verifyPassword(password, stored) {
    ...
    // Timing-safe comparison on raw bytes (not base64 strings)
    const computed = new Uint8Array(hash);
    const expected = _base64ToUint8(expectedHash);
   ...
   */
  _ensureInit() {
    if (!this._users || !this._sessions) throw new Error('Auth not initialized. Call await auth.init() first.');
  }
