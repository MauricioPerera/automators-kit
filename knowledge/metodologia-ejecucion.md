...
1. **PLAN** — convertir el pedido en contrato de ejecución con tareas atómicas; mostrarlo
   ...
   **Las afirmaciones de estado del entorno DENTRO de la spec son suposiciones del plan**
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#entorno-afirmado-plan--recon)).
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#crear-sobre-existente-plan--recon)).
   **Cuando el espacio de "qué falta hacer" es grande y abstracto, un artefacto real
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#artefacto-real-encuentra-gaps-que-la-clasificacion-abstracta-no-plan)).
2. **SPEC por tarea** — autocontenida y por OBJETIVO (estado final + definición de hecho
   ...
   **Red-team de la definición de hecho antes de delegar:** preguntar «¿cómo podría
   ...
   paso previene están en [casos reales](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#hecho-sin-intencion-spec--red-team-de-la-definición-de-hecho).
   ...
   **Los nombres de entregables se verifican como libres antes de asignarlos** (`ls` del
   ...
   existe, no lo sobrescribas» ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#colision-de-entregables-spec)).
   **Si el orquestador conoce una tensión de diseño fundamental de la tarea, va EN la
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#tension-de-diseno-en-la-spec-spec)).
   **Cambio de formato en un artefacto persistente** (journal, wire format, schema): la
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#formato-persistente-sin-contrato-de-upgrade-spec--cierre)).
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#nota-de-upgrade-no-ejecutada-spec--verificar)).
   **Una clase de fallo cazada dos veces se convierte en cláusula estándar** de toda spec
   ...
   verificación ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#clase-cazada-a-clausula-de-spec-spec)).
   **Si el comportamiento a cambiar está descrito en un contrato documentado, docs, código
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#contrato-documentado-cambia-junto-spec--verificar)).
   **En cambios que prometen preservar comportamiento** (refactor, unificación,
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#excepcion-unica-declarada-spec)).
   **Al agregar a un compilador/gramática compartida un nodo con una propiedad atípica**
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#propiedad-atipica-en-compilador-generico-spec)).
3. **DELEGAR** — un agente efímero por tarea. Tareas que compartan archivos → secuenciales.
   ...
   **Una credencial efímera puede viajar al agente delegado** solo si muere con la infra,
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#credencial-efimera-a-delegados-delegar)).
   **Ante una muerte del agente a mitad de tarea** (cuota, crash): triar el disco — lo
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#rescate-hibrido-por-cuota-delegar--recuperación)).
   **Si se delega investigar la viabilidad de algo riesgoso aceptando BLOQUEADO como
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#investigacion-bloqueada-necesita-arnes-delegar)).
4. **VERIFICAR por artefacto** — la palabra del agente no cuenta: solo salidas reales de
   ...
   **El orden es verificar → limpiar, nunca limpiar → verificar:** el artefacto de prueba
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#verificar-antes-de-limpiar-verificar)).
   **Un fix a una función que un lado de un contrato bilateral produce y otro consume
   ...
   consume mi output" ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#contrato-bilateral-mitad-arreglado-verificar)).
   **Cuando varios auditores de dominio corren sobre el mismo target sin comunicarse
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#dos-dominios-de-capa-3-que-nunca-se-hablan-convergen-en-el-mismo-riesgo-verificar)).
   **Una verificación de ausencia solo vale si la herramienta corrió de verdad:**
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#check-que-fallo-no-es-check-verificar)).
   **Antes de confiar en un gate para cerrar un ciclo, leer qué CHEQUEA REALMENTE
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#alcance-real-de-un-gate-no-es-el-prometido-verificar)).
   **Toda edición programática de documentación se verifica con grep de PRESENCIA
   ...
   silencio
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#replace-silencioso-en-docs-cerrar--documentación)).
   **La re-demostración del orquestador usa la invocación DOCUMENTADA**, no la que el
   ...
   esa diferencia ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#demo-por-el-camino-documentado-verificar)).
...
6. **CIERRE** — suite completa 2× (dos corridas idénticas ≈ sin flaky; un flaky detectado
   ...
   **Tras una interrupción, la infraestructura huérfana es evidencia antes que basura:**
   ...
   desmontarla ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#infra-huerfana-es-evidencia-cierre)).
   ...
   **El dueño de este paso es quien cierra, no quien declaró** — el ejecutor ya cumplió su
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#declarar-un-residuo-no-lo-registra-cierre)).
...
4. Repetir 2-3 hasta que una ronda de confirmación vuelva **completamente en blanco**
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#ronda-de-confirmacion-cierre)).
   **El ciclo no exige un motor vivo.** Cuando el cambio a confirmar no tiene código ni
   ...
   ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#auditoria-de-artefacto-sin-motor-vivo-cierre)).
...
- El «PERSISTE» o «imposible» de un auditor se re-verifica con reproducción barata del
  ...
  ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#persiste-de-auditor-refutado-verificar)).
- Los «NO VERIFICADO» que se repiten entre rondas por falta de una dependencia son
  **deuda de auditoría**, no una anotación neutral — la ronda siguiente provisiona la
  ...
  ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#no-verificado-acumulado-verificar)).
- Cuando un fix documenta un límite/alcance explícito, la ronda de confirmación
  ...
  ([caso real](https://github.com/MauricioPerera/KDD/blob/main/knowledge/casos-reales.md#limite-declarado-es-el-siguiente-objetivo-de-auditoria-verificar)).
