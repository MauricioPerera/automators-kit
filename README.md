...
1451 tests | 0 deps | 27 modules | Bun + Deno + Node.js
...
Six plugins ship in `plugins/`: `webhooks`, `automations`, `revisions`,
`audit`, `search`, `scheduler`. They are **opt-in** — nothing loads unless
`opts.plugins` names them, so a default `createApp()` runs none of them.
Full route table in [AGENTS.md](AGENTS.md#bundled-plugins-and-their-route-auth).

Two things to know before enabling any of them:

- **Their routes are `admin`-gated at the mount point.** Until 2026-08-04 they
  had no auth at all — an anonymous caller could register an outbound webhook
  and receive every entry created from then on (Known Security Gaps item 31).
  A plugin can widen a specific route with `publicRoutes`/`authRoutes`; only
  `webhooks` does, for its inbound receiver.
- **Outbound URLs go through the SSRF guard.** A webhook or automation action
  pointed at `127.0.0.1` is refused, matching every other config-driven
  outbound call here. Worth knowing if you were aiming one at a local service
  during development.

Writing your own plugin: reach for the core helpers rather than rewriting
them. Every plugin finding came from a plugin doing by hand something `core/`
already did carefully — use `safeFetch` (`core/net-guard.js`) for outbound
calls, `validateQuery` + `LIST_LIMIT_SCHEMA` + `clampLimit` (`core/validate.js`)
for `?limit=`, and `getNestedValue` (`core/db.js`) for dot-paths. That last one
**throws** on a `__proto__`/`constructor`/`prototype` segment, so catch per item
if you call it in a loop.
...
```bash
bun test tests/    # 1451 tests across 85 files, ~35 seconds
```
...
rather than left implied by a blanket "all remediated": `net-guard` now resolves hostnames, but cannot
stop **DNS rebinding** (it checks a name, then hands the name to `fetch`, which resolves again), and a
set of auditor-reported leads were **not** independently reproduced here and are recorded as
leads rather than conclusions. Both are listed in
[AGENTS.md's Known Security Gaps](AGENTS.md#known-security-gaps-items-1-37-resolved-open-items-listed-at-the-end).
...
- **2026-08-04 (`onExecutionFinished`: a shared execution history that sees every run)**: found sweeping for written-but-unwired code. The first diagnosis was wrong and is worth stating: `integrations/postgres-execution-log.js` is *not* missing engine wiring by oversight — its own doc comment says "NOT a change to core/workflow.js" and explains why (`db.js`'s storage-adapter interface is synchronous and `Collection` caches everything in memory). It is a deliberate caller-driven sidecar. The real gap is sharper: the usage it documents — `await log.record(await engine.execute(...))` — is sound for a manual run and **structurally cannot see** the executions that dominate a real deployment, since trigger-fired (webhook/cron/poll) and error-workflow runs go through `_dispatchExecution` fire-and-forget, where nothing receives the execution object. A multi-worker setup following those instructions would get a shared history containing only manually-run executions. Fixed with a **generic** `onExecutionFinished` callback rather than a `postgresExecutionLog` option, so the engine keeps no Postgres knowledge — the same reason that module gives for not touching the file — and the same seam serves alerting, custom logging or metrics. Fires only for terminal statuses (a paused execution is not finished), passes an execution with a resolvable `_id`, and is fire-and-forget: a throwing or rejecting observer is logged and never fails the execution. Verified end-to-end against a real Postgres in the shape the module was written for — two engines with independent local stores funnelling into one shared table, with both the manual and the webhook-triggered run arriving, and `nodeResults` surviving the JSONB round trip. 7 local regression tests.
- **2026-08-04 (table templates made reachable — and persistable at all)**: `TEMPLATES`/`createFromTemplate` defined four ready schemas (`crm`, `tasks`, `inventory`, `content`) with typed columns, `unique` constraints, select options and defaults, and **nothing could reach them**: the data-table surfaces resolve schemas through the registry, which `createFromTemplate` never touches. Wiring it revealed they **could not have worked anyway** — their `CreatedAt` columns used `default: () => new Date().toISOString()`, and the registry persists columns as JSON, so `Collection.insert()`'s `structuredClone` threw `DataCloneError` and registering any template crashed. Two fixes: a serializable `'$now'` sentinel for dynamic defaults (a `Table` built in code still honours a function default, unchanged), and `setTableSchema` now refusing a function default with the reason and the alternative instead of letting a `DataCloneError` surface from inside `insert()`. Integration goes through the existing registry, so templates constrain **both** surfaces: `GET /api/db/_templates` to discover, `PUT /api/db/:col/_schema { template: 'inventory' }` to apply, and the `data.table` node enforces the same contract — verified that required and unique constraints from a template are enforced over HTTP and from a workflow alike. Known narrow limitation, documented in code: a column wanting the literal string `"$now"` as its default cannot express it. 12 new regression tests. This is the second time in this sweep that wiring disconnected code revealed it could never have worked, after the metrics labels.
- **2026-08-04 (query params validated, and a row cap that only bounded one side)**: last item of the written-but-unwired sweep. `core/validate.js` exported `validateQuery` and **no route used it**, so every query param arrived unvalidated and uncoerced -- which is precisely why each route had hand-rolled its own `parseInt(...) || N`. One hand-roll was wrong in a way that mattered: `/api/db/:col`'s documented 500-row cap was `Math.min(parseInt(q._limit) || 50, 500)`, which bounds the TOP but not the BOTTOM. Any negative is already `<= 500`, so it passed through `Math.min` unchanged and reached `cursor.limit(-1)`, where the underlying `slice` reads a negative length as "no limit at all". Reproduced live over the real routes on a 2000-row collection: `?_limit=99999` correctly returned 500 rows, `?_limit=-1` returned **all 2000** -- any authenticated user, including a freshly self-registered `viewer`, could dump an entire collection in one request. Two smaller defects shared the root cause: `?_limit=1e9` returned a **single** row (`parseInt` stops at the `e`), and `/api/shell/history?limit=-5` returned 25 of 30 entries instead of 5 (negative `slice`, counted from the wrong end). Now wired into the three limit-taking GET routes, following the existing `validateBody` -> `ctx.state.body` convention. Two deliberate non-changes: `_limit` has **no** `$max`, so an over-cap request is still silently clamped exactly as before rather than newly returning 400; and `stripUnknown` stays off, because every non-reserved param is a dynamic filter field (`?status=draft`, `?age__gt=18`) and dropping one would silently return **unfiltered** rows -- a worse failure than rejecting the request. One visible behavior change: `_order` is now an `asc|desc` enum, where any non-`asc` value previously meant descending. Why it survived: **no test touched `_limit`/`_offset`/`_sort`/`_order`/`_fields` at all**. 12 new tests, 7 of which fail against the pre-fix routes (verified by stashing `routes/`).
- **2026-08-04 (what six disconnected pieces taught -- and one correction)**: this sweep looked for "a mechanism exists and nothing drives it" and found six. Wiring them exposed a latent bug in **four**: unbounded metric label cardinality leaking ids into an unauthenticated endpoint, `purgeExecutions` deleting live executions, template defaults that made a template impossible to register, and the pagination cap above. All four were invisible to the six-agent security audit run earlier -- an auditor reads what code does, not what happens when it finally runs. **The correction, because the tidy version is wrong:** `validateQuery` was *not* dormant code. It had a real consumer all along in `examples/api-validation/setup.js`, covered by its own test -- so unlike `TEMPLATES` or the metrics chain, it was written, wired and proven. It was unused by `routes/` specifically, which is a different failure: the production routes each hand-rolled `parseInt(...) || N` instead of reaching for the tested helper next to them. The bug was in the ad-hoc copies, not in unexecuted code. So there are two shapes here, not one: **(1) nothing drives this mechanism** -- an export with no consumer anywhere has never run and is often broken; **(2) something drives it, but not the code that matters** -- a helper used by an example while the security-relevant surfaces reimplement it by hand. Shape 2 is the harder hunt, because the reassuring signal (grep finds the symbol in use) is exactly what hides it. It is also the same shape as the `data.table` node and `/api/db/:col` drifting apart earlier in this audit, which is what made the node a second path to privilege escalation.
- **2026-08-04 (hunting the second shape on purpose: bundled plugins were wide open)**: instead of leaving "shape 2" as an observation, this pass went looking for it deliberately -- *which proven helper should this code be using and is not?* It concentrated in one place: `plugins/`, where all six bundled plugins imported only `core/http.js`, had **no tests whatsoever**, and were documented nowhere (all three since addressed). Three findings. **(1) CRITICAL -- plugin routes were completely unauthenticated.** Routers were mounted raw and no plugin registered middleware of its own. Reproduced live with no `Authorization` header anywhere: `POST /api/plugins/webhooks/` registered an outbound webhook aimed at `http://127.0.0.1:<port>/admin`, and creating an entry made the server POST that entry's **full content** to it; the same worked through `plugins/automations`, delivering `{"leaked":"secret draft"}`. Point it at an attacker host instead of loopback and it is a **persistent exfiltration channel any anonymous caller can install**. Gated at the mount point rather than per plugin, so a plugin -- bundled or third-party -- cannot ship unauthenticated by forgetting something; default `admin`, since these routes name outbound destinations and a viewer who could still register one would leave most of the finding intact. Two opt-outs (`publicRoutes`, `authRoutes`) exist because the inbound webhook receiver genuinely must stay open, being called by an external service with no token -- the same case as the OAuth2 callback. A malformed declaration throws at mount time rather than being skipped. **(2) HIGH -- raw `fetch` instead of `safeFetch`** in both outbound-calling plugins, which is what let (1) reach loopback; `net-guard` was already exported, tested and used by five core modules. This also made a README claim true that had been false: the SSRF guard covering "all outbound fetches driven by workflow/trigger definitions". **(3) MEDIUM -- `checkCondition` failed open**, `default: return true`, so a typo'd operator made the condition pass and fired the action against records it was written to exclude. Scope, plainly: plugins are opt-in via `opts.plugins`, so a default `createApp()` never ran any of this -- but they ship as usable plugins with no warning. 13 new tests (the six plugins had none, which is why the six-agent audit missed all of it); 9 of the 13 fail against the pre-fix code.
- **2026-08-04 (the two lessons from that hunt)**: **the question that finds shape 2 is not greppable.** Shape 1 answers to "what has no consumer?", which a grep resolves in seconds; here `safeFetch` had five consumers and looked healthy, and the useful question was "who *should* be calling this and isn't?" -- which no command answers, and which has to be asked surface by surface. **And fixing it required exporting a private helper, the same root cause again.** The gate needed to match route patterns and `compilePattern` was module-private, so hand-rolling a second matcher would have recreated the exact divergence being fixed; it was exported instead. That is the `_getNestedValue` lesson applied rather than recorded: a hardened helper nothing outside its module can reach does not get reused, it gets reimplemented -- badly, and without the hardening. If a helper exists because it is careful about something, export it, or accept that every caller outside its module will write a careless version.
- **2026-08-04 (an unbounded `Wait` on a deliberately public endpoint)**: applying the same shape-2 question to the remaining exported security helpers turned up one real defect. `a2e.js`'s `Wait` was `setTimeout(resolve, config.duration || 0)` with **no upper bound**, and `POST /api/a2e/execute` is documented `auth: 'none'` -- public by design, per `GET /api/schema`'s own catalog. Verified over real HTTP with **no Authorization header**: a one-operation workflow asking for 4000ms held the request for 4020ms, and nothing stopped it asking for 24 hours. An anonymous caller could pin server resources for as long as it liked, a few bytes of request at a time. Fixed with `maxWaitMs`, following the `maxDepth ?? 50` guard two lines above it in the same constructor: default 30s, configurable per executor, `0` disables. Configurable rather than hard-coded, since a hard limit with no escape hatch is what this codebase already declined to do to `connector.js`'s internal-host guard. It **refuses rather than clamps** -- a shorter-than-requested pause would leave the author believing something happened that did not. Negative, non-numeric and `Infinity` durations are refused too; `setTimeout` accepted all three and fired immediately. Handlers now get a third argument carrying executor limits, purely additive, with a test asserting a custom `registerHandler((config, state) => ...)` still works, since breaking that would have been a silent cost of the fix. **This is a defect within the public-by-design decision, not an argument against it.** 11 new tests; 6 of 10 in the new block fail without the cap.
- **2026-08-04 (why a mostly-negative sweep is the result, not a wasted pass)**: nine surfaces were examined in that sweep and **eight were clean**. The two duplicated webhook-secret checks have not drifted (the best drift candidate in the repo, and it held); JWT verification is constant-time via `crypto.subtle.verify`; `updateProject` whitelists at the model layer, so `PUT /api/projects/:id` handing it a raw body cannot rewrite `members`; `requirePermission` is applied consistently, and `content-types`' `requireRole('admin')` is the established structural-changes-are-admin decision rather than an inconsistency. After three serious findings in `plugins/`, the tempting conclusion was that the repo was full of this -- it was not, the problem was concentrated in one directory. Recording that stops the next reader treating every duplicated-looking helper as suspect. One more thing earned its keep: **`GET /api/schema` worked as an oracle.** `routes/shell.js` and `routes/a2e.js` have no auth on any route and mount unconditionally, which looked exactly like the plugin hole; the catalog resolves it by declaring `auth: 'none'` per endpoint and saying of the shell "All routes public by design". A catalog that records the auth requirement per endpoint is what lets "deliberate decision" be told from "oversight" by checking rather than guessing.
- **2026-08-04 (closing the two deferred plugin items -- and what replacing a weak copy costs)**: four plugins' unbounded `?limit=` and `plugins/automations`'s unguarded dot-path getter, both known and deliberately left when the plugin security fixes shipped. Two things this pass taught that the earlier ones did not. **Four copies of one rule produced four DIFFERENT behaviours:** `?limit=-1` returned *every* row through `Cursor.limit(-1)` in three plugins, and *every row but the last* through `results.slice(0, -1)` in `search`. The copies had not merely duplicated a bug, they had each grown their own -- which is the concrete argument for a shared decision point over a shared convention. So the fix exports `LIST_LIMIT_SCHEMA`/`clampLimit()` once from `core/validate.js` instead of adding four more identical schemas; fixing duplication by creating duplication would have missed the point. **And swapping in a hardened helper is not mechanical -- it inherits the original's failure behaviour.** `core/db.js`'s guarded getter THROWS on a `__proto__`/`constructor`/`prototype` segment where the plugin's weak copy silently returned a value, and `matchFilter` is called from a hook loop that is *not* inside a `try` -- so the drop-in would have let one workflow with a bad filter path abort the hook for every OTHER workflow on that event, turning a harmless read bug into an availability failure. Caught by reading the call site before shipping, fixed with a per-workflow catch (skip and log the offender, keep the rest running), verified live. When you replace a permissive helper with a strict one, the call sites need re-reading, not just re-pointing. 8 new tests; 4 of the 5 limit tests fail pre-fix, and of the three path tests **only one does** -- the trigger-filter one passes against the old code, since an unguarded `constructor.prototype` simply did not match and the workflow was skipped anyway. That test guards the `try`/`catch` added here, not the original defect; counting all three would have overstated it.
- **2026-08-04 (the DNS hole closed -- and the scope note replaced, not deleted)**: `net-guard` had disclosed from its first line that it performed no DNS resolution, and that disclosure survived two rounds of guard fixes without being acted on. It resolves now: a public-looking hostname whose A record points at `127.0.0.1` or `169.254.169.254` is refused, on the initial URL and every redirect hop, checking **every** address a name returns rather than the first (a name answering with one public and one private address is exactly how a first-hit check gets slipped past). Verified live against real resolution: `safeFetch('http://localtest.me/')`, a genuine public hostname that resolves to loopback, is blocked with `resolves to ::1`; public names still pass. **What it does not close: DNS rebinding.** The check resolves a name, then hands the NAME to `fetch`, which resolves again independently -- a 0-TTL record answering public-then-private still wins, and pinning the verified IP needs a `Host` header plus a TLS SNI override that is not reachable portably with zero dependencies across Bun, Deno and Node. Three decisions inside the fix: the check lands in `safeFetch` rather than the synchronous `assertPublicUrl` (whose callers include ones that validate a stored URL without ever making a request, and would all have needed async signatures); `assertPublicLiteral` was extracted so resolved addresses face the identical rule, since writing a second "is this internal" is the mistake the plugin items fixed four times over; and on a runtime with no `node:dns` (Cloudflare Workers) the check is **skipped** deliberately, because failing closed would block every outbound request there -- an outage, not a hardening. 13 new tests using an injected resolver, so the suite needs no network and works offline. **A cost found only by running the full suite after this shipped:** resolve-then-fetch resolves TWICE, because there is no portable way to hand `fetch` an already-resolved address. The OS resolver caches (~1ms steady state), but the first call to any host pays a real round-trip -- ~800ms measured against a cold resolver. That was not hypothetical: it made two poll-trigger example tests flaky immediately (1 fail / 0 fail / 1 fail across three runs), since their placeholder host was suddenly being resolved for real on every poll; both now inject a resolver. No app-level cache was added on purpose -- caching a security check's result means a stale answer can allow a destination current DNS would refuse.
- **2026-08-04 (why the scope note was rewritten instead of removed)**: the tempting move after closing most of a hole is to quietly upgrade the docs to "covered". The old note said `net-guard` does no DNS resolution; the new one says it resolves but cannot stop DNS rebinding, and explains exactly why that was not attempted. **Redraw the line, do not erase it** -- anyone reading the module now knows which attack still works, instead of believing DNS is handled. The same applies to the fail-open when `node:dns` is missing: recorded as a decision with its reason, rather than left as an unexplained gap someone later "fixes" by making it strict and breaking a supported runtime. One measurement note in the same spirit: unlike the earlier items, these tests **cannot** be run against the pre-fix code to show them failing, because the file does not compile without the new exports. That is expected for new API surface, but the usual "N of M fail pre-fix" evidence is genuinely unavailable here, and claiming it would have been false.
...
- SSRF guard (`net-guard.js`) on all outbound fetches driven by workflow/trigger definitions **and by the bundled plugins**, covering IPv4 and IPv6 (including IPv4-mapped forms and unique-local `fc00::/7`), re-validating every redirect hop, and resolving hostnames to reject names that point at internal addresses; **does not stop DNS rebinding**
...
- `a2e.js`'s `Wait` capped at 30s by default (`maxWaitMs`, `0` disables) — unbounded until 2026-08-04, on an endpoint that is public by design
