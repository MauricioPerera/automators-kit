...
1341 tests | 0 deps | 27 modules | Bun + Deno + Node.js
...
| Module | What it does |
|--------|-------------|
| **db.js** | Document DB: MongoDB queries, 26 operators, indices, JWT auth, AES-256-GCM encryption, proxy access, watch, long-lived API keys (separate from a login session) |
...
```bash
bun test tests/    # 1341 tests across 84 files, ~35 seconds
```
...
4 full security audits to date. **Every finding that was independently reproduced is remediated** — 18
from the 2026-08-03 full-codebase audit alone (4 critical, 7 high, 7 medium). That includes all nine of
its initially-unreproduced leads, every one of which turned out to be a real bug once checked. What is
still unverified is the Postgres integrations, which need a live Postgres this environment does not
have — listed as unverified rather than quietly counted as done. What remains open is stated
...
[AGENTS.md's Known Security Gaps](AGENTS.md#known-security-gaps-items-1-25-resolved-open-items-listed-at-the-end).
