...
1038 tests | 0 deps | 26 modules | Bun + Deno + Node.js
...
| **workflow.js** | Workflow engine: n8n-style nodes, triggers, credentials, execution history, DAG-parallel execution, N-way branching (`switch`/`runIf`), error workflows, sub-workflows (`workflow.execute`), persisted wait (`wait.until`/`wait.forWebhook`), per-item processing (`loop.forEach`) |
...
```bash
bun test tests/    # 1038 tests across 82 files, ~34 seconds
```
