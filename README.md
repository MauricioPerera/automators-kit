...
1284 tests | 0 deps | 27 modules | Bun + Deno + Node.js
...
```bash
bun test tests/    # 1284 tests across 84 files, ~35 seconds
```
...
4 full security audits to date. Every critical and high finding is remediated; the 2026-08-03
full-codebase audit also surfaced confirmed medium-severity issues that are **not** fixed yet — they are
listed in [AGENTS.md's Known Security Gaps](AGENTS.md#known-security-gaps-items-1-14-resolved-open-items-listed-at-the-end) under "Other
audit findings confirmed but not yet fixed", rather than left implied by a blanket "all remediated".
