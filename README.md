...
| **workflow.js** | Workflow engine: n8n-style nodes, triggers, credentials, execution history, DAG-parallel execution, N-way branching (`switch`/`runIf`), error workflows, sub-workflows (`workflow.execute`), persisted wait (`wait.until`) |
...
CSV node, `core/nodes.js`'s 20 built-ins don't. Instead of waiting for
