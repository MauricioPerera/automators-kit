...
1127 tests | 0 deps | 27 modules | Bun + Deno + Node.js
...
| **workflow.js** | Workflow engine: n8n-style nodes, triggers, credentials, execution history, DAG-parallel execution, N-way branching (`switch`/`runIf`), error workflows, sub-workflows (`workflow.execute`), persisted wait (`wait.until`/`wait.forWebhook`), per-item processing (`loop.forEach`), per-node retry/backoff |
...
```bash
bun test tests/    # 1127 tests across 83 files, ~35 seconds
```
...
- **2026-08-03 (independent second-opinion audit — 2 unresolved security gaps found, not yet fixed)**: delegated a fresh, no-prior-context audit (a separate model instance given only a clean clone of the repo, no knowledge of any work done building the features it was auditing) to independently re-run the test suite, live-test the platform, and redo the n8n comparison from scratch. Test suite: 1154 pass / 5 skip / 0 fail, stable across 2 runs. Live testing (real server, real curl, real MCP stdio): 0 bugs in the core flows tested. Two real findings, independently re-verified by reading the exact source afterward — **documented in [AGENTS.md's "Known Security Gaps (unresolved)"](AGENTS.md#known-security-gaps-unresolved), not fixed yet, on request**: (1) `POST /api/auth/register` lets an unauthenticated caller set `role: 'admin'` directly, no gate anywhere between the route and `UserService.register()`; (2) `GET /api/workflows/:id` and `POST /api/workflows/:id/run` require only `auth`, no project-membership check, so any authenticated instance user can read or run any workflow regardless of project.
