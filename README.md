...
1438 tests | 0 deps | 27 modules | Bun + Deno + Node.js
...
```bash
bun test tests/    # 1438 tests across 85 files, ~35 seconds
```
...
[AGENTS.md's Known Security Gaps](AGENTS.md#known-security-gaps-items-1-36-resolved-open-items-listed-at-the-end).
...
- SSRF guard (`net-guard.js`) on all outbound fetches driven by workflow/trigger definitions **and by the bundled plugins**, covering IPv4 and IPv6 (including IPv4-mapped forms and unique-local `fc00::/7`) and re-validating every redirect hop; does not cover DNS resolution
