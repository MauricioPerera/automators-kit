...
1298 tests | 0 deps | 27 modules | Bun + Deno + Node.js
...
```bash
bun test tests/    # 1298 tests across 84 files, ~35 seconds
```
...
4 full security audits to date. Every critical, high and medium finding that was independently reproduced
is remediated. What remains open is stated rather than left implied by a blanket "all remediated": one
known SSRF gap (redirects — see the 2026-08-03 net-guard entry below) and a set of auditor-reported leads
that were **not** independently reproduced here, both listed in
[AGENTS.md's Known Security Gaps](AGENTS.md#known-security-gaps-items-1-17-resolved-open-items-listed-at-the-end).
...
- SSRF guard (`net-guard.js`) on all outbound fetches driven by workflow/trigger definitions, covering IPv4 and IPv6 (including IPv4-mapped forms and unique-local `fc00::/7`); does not cover redirects or DNS resolution
